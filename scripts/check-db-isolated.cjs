...
try{
 ...
 for(const file of ['tenant-isolation.sql','customer-portal.sql','global-admin.sql','notifications.sql','subscription-lifecycle.sql','security-hardening.sql','rose-rocket.sql','tms-connections.sql','tms-automatic.sql','exclusive-intake.sql','evidence-validation.sql'])sql+='\n'+readFileSync(join(root,'tests/db',file),'utf8');
 sql+='\nROLLBACK;';
