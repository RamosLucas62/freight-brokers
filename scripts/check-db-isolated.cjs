...
try{
 ...
 for(const file of ['tenant-isolation.sql','customer-portal.sql','global-admin.sql','notifications.sql','subscription-lifecycle.sql','security-hardening.sql','rose-rocket.sql','tms-connections.sql','tms-automatic.sql'])sql+='\n'+readFileSync(join(root,'tests/db',file),'utf8');
 sql+='\nROLLBACK;';
