...
const productGuideSteps=[
 {section:'DOCUMENT SETUP',title:'How will your documents arrive?',description:'Choose your TMS to connect automatic document delivery. If you do not use a TMS or yours is not supported, we will guide you through email intake.',points:['Email stays available until a complete scan and every discovered load pass the evidence checks.','Report emails continue normally.','Disconnecting the last TMS restores email intake.'],stage:'Connect'},
...
function renderGuideIntakeChoice(){
 ...
 const connections=guideProviders.filter(p=>p.connection?.status==='verified'&&p.connection?.sync_enabled);const active=connections.length>0&&connections.every(p=>p.connection.intake_validated_at)?connections[0]:null;
 ...
 const draw=()=>{guideSelection=$('guide-tms').value;const provider=guideProviders.find(p=>p.id===guideSelection);const email=guideSelection==='email'||(provider&&(provider.mode==='unavailable'||!provider.setup_available));
  $('guide-intake-note').textContent=active?'Your TMS import is active. Email intake is disabled. Manage or disconnect your connections in Settings.':email?'We will show your private email address, authorized senders and how to send your first PDF.':provider?`${provider.instructions} Email intake turns off only after every active connection completes its scan and all discovered loads have validated evidence.`:'Select your system to see the next step.';
 ...
 try{
  ...
  const draw=()=>{
   ...
   if(provider.id==='rose-rocket'){detail.innerHTML=roseIntegrationCard({...rose,intake_validated_at:provider.connection?.intake_validated_at});if(provider.connection?.sync_enabled)detail.insertAdjacentHTML('beforeend',`<p class="muted">${provider.connection.sync_error?'Sync needs attention. Reconnect after checking API access.':provider.connection.last_synced_at?`Last checked ${escape(date(provider.connection.last_synced_at))}`:'Waiting for the first automatic check.'} · ${Number(provider.connection.imported_batches??0)} document batches queued</p>`);bindRoseConnection();}
    ...
    const label=state==='verified'?(provider.connection.sync_error?'Sync needs attention':provider.connection.sync_enabled?(provider.connection.intake_validated_at?'Collection coverage validated':'Collecting · evidence validation pending'):'Import paused'):provider.mode==='unavailable'?'Not available':'Not connected';
    const note=state==='verified'?(provider.connection.sync_enabled?(provider.connection.intake_validated_at?'Collection coverage validated for this connection. Each new batch still needs matching freight evidence. Email turns off when every active connection is ready; report emails continue.':'Documents are being collected. Email remains available until a complete scan has no missing documents or loads awaiting evidence validation. Missing evidence appears in the queue.'):'Enable automatic import to receive documents from this connection.'):provider.mode==='unavailable'?'This connector cannot be activated yet. Olympian needs to complete the vendor integration before it can be offered.':'Connecting starts collection; email stays available until a complete scan and every discovered load pass the evidence checks. Connect to start importing documents automatically. Tai imports carrier bills and their supporting PDFs; McLeod checks documents on delivered orders.';
...
function bindRoseConnection(){
 ...
 $('rose-connection-form')?.addEventListener('submit',async event=>{
  ...
  try{const fields=Object.fromEntries(new FormData(form));if(!fields.history_board_id)delete fields.history_board_id;await api(`integrations/rose-rocket?company=${encodeURIComponent(company)}`,{method:'POST',body:JSON.stringify(fields)});form.reset();if(form.isConnected&&$('company').value===company)await load(false,{force:true});}
  catch(error){if(feedback.isConnected){feedback.textContent=error.message;button.disabled=false;}}
 });
...
function detail(row){
 ...
 if(view==='jobs'&&row.evidence_issues?.length)html+=`<h3>Evidence needed</h3><ul>${row.evidence_issues.map(issue=>`<li>Load ${escape(issue.load??'not identified')}: ${escape(issue.missing.map(code=>({CARRIER_INVOICE:'carrier invoice',INVOICE_LOAD_NUMBER:'invoice load number',INVOICE_LOAD_REVIEW:'verify invoice load number',INVOICE_DETAILS:'invoice details',POD:'matching POD',SIGNED_POD:'readable signed POD',DELIVERY_DATE:'delivery date',RATE_CONFIRMATION:'matching rate confirmation',AUTHORIZED_TOTAL:'authorized total',POD_REVIEW:'POD review',RATE_CONFIRMATION_REVIEW:'rate confirmation review',AMBIGUOUS_POD:'resolve multiple matching PODs',AMBIGUOUS_RATE_CONFIRMATION:'resolve multiple matching rate confirmations',ACCESSORIAL_UNSUPPORTED_TERMS:'unsupported charge terms',ACCESSORIAL_AMBIGUOUS_CHARGES:'repeated charge lines',ACCESSORIAL_CHARGE_SOURCE:'readable invoice charge amount',ACCESSORIAL_INVOICE_CURRENCY:'explicit invoice currency',ACCESSORIAL_AMBIGUOUS_RECEIPT:'resolve multiple matching receipts',ACCESSORIAL_RECEIPT:'matching receipt or time record',ACCESSORIAL_RECEIPT_REVIEW:'readable receipt evidence',ACCESSORIAL_SERVICE_DATE:'service date matching the delivery',ACCESSORIAL_RECEIPT_CURRENCY:'receipt currency matching the invoice',ACCESSORIAL_AMBIGUOUS_AUTHORIZATION:'resolve multiple authorizations',ACCESSORIAL_AUTHORIZATION:'explicit authorization for this carrier',ACCESSORIAL_AUTHORIZATION_CURRENCY:'authorization currency matching the invoice',ACCESSORIAL_AUTHORIZATION_DATE:'authorization for this service date',ACCESSORIAL_TIME_TERMS:'explicit hourly rate, free time and rounding terms',ACCESSORIAL_TIME_RANGE:'valid arrival and departure times',ACCESSORIAL_TIME_CAP:'verified maximum charge',ACCESSORIAL_AUTHORIZED_AMOUNT:'explicit authorized amount',ACCESSORIAL_RECEIPT_AMOUNT:'receipt amount matching the invoice',ACCESSORIAL_AMOUNT_MISMATCH:'billed amount matching the authorized calculation',ACCESSORIAL_DOCUMENT:'additional-charge evidence with a readable matching load reference',ACCESSORIAL_EVIDENCE_REVIEW:'manual review of additional-charge receipts and terms'}[code]??code)).join(', '))}</li>`).join('')}</ul>`;
 ...
 for(const check of report.accessorial_checks??[])html+=`<div class="exception"><strong>${escape(check.type)} (${escape(check.currency??'currency unverified')}) · ${check.status==='verified'?'Evidence verified':'Review required'}</strong><p>Billed ${money(check.billed)} · Expected ${check.expected==null?'Not established':money(check.expected)}</p><small>${escape(check.sources.join(', '))}</small></div>`;
