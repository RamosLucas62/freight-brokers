...
function roseIntegrationCard(rose){
 ...
 const state=status==='active'?['Automatic import enabled','PDFs attached to orders are received after order-status updates and checked again automatically. Existing orders need a status update before their first import.']:status==='pending'?['Setup incomplete','Reconnect to enable automatic document delivery.']:['Not connected','Your email intake works without a TMS connection.'];
 ...
 try{
  ...
  const draw=()=>{
   ...
   if(provider.id==='rose-rocket'){detail.innerHTML=roseIntegrationCard(rose);if(provider.connection?.sync_enabled)detail.insertAdjacentHTML('beforeend',`<p class="muted">${provider.connection.sync_error?'Sync needs attention. Reconnect after checking API access.':provider.connection.last_synced_at?`Last checked ${escape(date(provider.connection.last_synced_at))}`:'Waiting for the first automatic check.'} · ${Number(provider.connection.imported_batches??0)} document batches queued</p>`);bindRoseConnection();}
    ...
    const label=state==='verified'?(provider.connection.sync_error?'Sync needs attention':provider.connection.sync_enabled?'Automatic import enabled':'Import paused'):provider.mode==='unavailable'?'Not available':'Not connected';
    const note=state==='verified'?(provider.connection.sync_enabled?'Documents are collected automatically and sent to audit. New batches appear in your processing queue.':'Enable automatic import to receive documents from this connection.'):provider.mode==='unavailable'?'This connector cannot be activated yet. Olympian needs to complete the vendor integration before it can be offered.':'Connect to start importing documents automatically. Tai imports carrier bills and their supporting PDFs; McLeod checks documents on delivered orders.';
