...
const productGuideSteps=[
 {section:'DOCUMENT SETUP',title:'How will your documents arrive?',description:'Choose your TMS to connect automatic document delivery. If you do not use a TMS or yours is not supported, we will guide you through email intake.',points:['An active TMS connection turns off email intake to avoid duplicate submissions.','Report emails continue normally.','Disconnecting the last TMS restores email intake.'],stage:'Connect'},
...
async function openProductGuide(){
 ...
 try{const result=await api(`integrations?company=${encodeURIComponent(company)}`);if(guideCompany!==company||$('company').value!==company)return;guideProviders=result.providers;guideSelection=guideProviders.find(p=>p.connection?.status==='verified'&&p.connection?.sync_enabled)?.id??'';renderGuideIntakeChoice();}
...
function renderGuideIntakeChoice(){
 ...
 const active=guideProviders.find(p=>p.connection?.status==='verified'&&p.connection?.sync_enabled);
 ...
 const draw=()=>{guideSelection=$('guide-tms').value;const provider=guideProviders.find(p=>p.id===guideSelection);const email=guideSelection==='email'||(provider&&(provider.mode==='unavailable'||!provider.setup_available));
  $('guide-intake-note').textContent=active?'Your TMS import is active. Email intake is disabled. Manage or disconnect your connections in Settings.':email?'We will show your private email address, authorized senders and how to send your first PDF.':provider?`${provider.instructions} Email intake turns off only after automatic import is activated.`:'Select your system to see the next step.';
...
function roseIntegrationCard(rose){
 ...
 const state=status==='active'?['Automatic import enabled · email intake off','PDFs attached to orders are received after order-status updates and checked again automatically. Existing orders need a status update before their first import. Email intake is disabled; report emails continue.']:status==='pending'?['Setup incomplete','Reconnect to enable automatic document delivery.']:['Not connected','Your email intake works without a TMS connection.'];
...
async function loadTmsPanel(rose){
 ...
 try{
  ...
  const draw=()=>{
   ...
   if(provider.id==='rose-rocket'){detail.innerHTML=roseIntegrationCard(rose);if(provider.connection?.sync_enabled)detail.insertAdjacentHTML('beforeend',`<p class="muted">${provider.connection.sync_error?'Sync needs attention. Reconnect after checking API access.':provider.connection.last_synced_at?`Last checked ${escape(date(provider.connection.last_synced_at))}`:'Waiting for the first automatic check.'} · ${Number(provider.connection.imported_batches??0)} document batches queued</p>`);bindRoseConnection();}
   else{
    const state=provider.connection?.status??'disconnected';
    const label=state==='verified'?(provider.connection.sync_error?'Sync needs attention':provider.connection.sync_enabled?'Automatic import enabled · email intake off':'Import paused'):provider.mode==='unavailable'?'Not available':'Not connected';
    const note=state==='verified'?(provider.connection.sync_enabled?'Documents are collected automatically and sent to audit. New batches appear in your processing queue. Email intake is disabled; report emails continue.':'Enable automatic import to receive documents from this connection.'):provider.mode==='unavailable'?'This connector cannot be activated yet. Olympian needs to complete the vendor integration before it can be offered.':'Connecting turns off email intake. Connect to start importing documents automatically. Tai imports carrier bills and their supporting PDFs; McLeod checks documents on delivered orders.';
