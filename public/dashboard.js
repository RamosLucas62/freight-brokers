...
function renderSettings(){
 ...
 $('settings-panel').querySelector('.settings-grid').insertAdjacentHTML('afterbegin','<section id="tms-panel" class="settings-card integration-card"><p role="status">Loading TMS connections…</p></section>');
 loadTmsPanel(rose);
...
async function loadTmsPanel(rose){
 ...
 try{
  ...
  panel.innerHTML=`<div><p class="eyebrow">DOCUMENT CONNECTIONS</p><h3>Connect your TMS</h3></div><p class="muted">Choose your system, follow the steps and test access here. Email intake stays available.</p><label for="tms-provider">Your transportation management system</label><select id="tms-provider">${result.providers.map(p=>`<option value="${escape(p.id)}">${escape(p.name)}</option>`).join('')}</select><div id="tms-provider-detail"></div>`;
  ...
  const draw=()=>{
   ...
   if(provider.id==='rose-rocket'){detail.innerHTML=roseIntegrationCard(rose);bindRoseConnection();}
   else{
    const state=provider.connection?.status??'disconnected';
    const label=state==='verified'?'Access verified':state==='requested'?'Setup requested':provider.mode==='assisted'?'Assisted setup':'Not connected';
    const note=state==='verified'?'Your API access was verified. Automatic document import is not available yet. Keep sending PDFs to your private intake.':state==='requested'?'Your request is saved in the Olympian setup queue. API access and document import still need to be configured.':provider.mode==='assisted'?'This provider needs an account-specific integration. You can request setup here.': 'Test and securely save your API access. This does not enable automatic document import.';
    const canConnect=provider.can_manage&&provider.setup_available;
    detail.innerHTML=`<div class="integration-heading"><h3>${escape(provider.name)}</h3><span class="integration-state ${state==='disconnected'?'':'pending'}">${label}</span></div><p class="muted">${note}</p><p>${escape(provider.instructions)}</p>${!provider.can_manage?'<p class="muted">Only the company owner or billing administrator can manage this connection.</p>':state==='verified'||state==='requested'?`<p>${escape(provider.connection.account_label??'Setup request saved')} · ${escape(date(provider.connection.verified_at??provider.connection.requested_at))}</p><button type="button" id="tms-disconnect">${state==='requested'?'Cancel request':'Disconnect'}</button>`:provider.mode==='assisted'?'<button class="primary" id="tms-request" type="button">Request setup</button>':canConnect?'<form id="tms-connect" class="integration-form" autocomplete="off"><div class="integration-fields"><label>Tai site code<input name="site" required maxlength="63" pattern="[A-Za-z0-9][A-Za-z0-9-]{0,62}" placeholder="yourcompany"><small>The part before .taicloud.net</small></label><label>API key<input name="api_key" type="password" required maxlength="2048" autocomplete="new-password"></label></div><button class="primary" type="submit">Test and save connection →</button></form>':'<p class="muted">Olympian needs to configure secure connection storage. Do not send API keys through support.</p><button type="button" data-tms-help>Get setup help</button>'}<p id="tms-feedback" role="status"></p><a class="integration-help" href="${escape(provider.helpUrl)}" target="_blank" rel="noopener noreferrer">${escape(provider.name)} setup reference ↗</a>`;
    const submit=async(action,button,fields={})=>{
     ...
     try{await api(`integrations/${provider.id}/${action}?company=${encodeURIComponent(company)}`,{method:'POST',body:JSON.stringify(fields)});if(!panel.isConnected||$('company').value!==company)return;$('tms-connect')?.reset();await loadTmsPanel(rose);}
     catch(error){if(feedback.isConnected){feedback.textContent=error.message;button.disabled=false;}}
    };
    $('tms-connect')?.addEventListener('submit',event=>{event.preventDefault();submit('connect',event.submitter,Object.fromEntries(new FormData(event.currentTarget)));});
    $('tms-request')?.addEventListener('click',event=>submit('request',event.currentTarget));
    $('tms-disconnect')?.addEventListener('click',event=>{if(confirm(state==='requested'?'Cancel this setup request?':'Remove the saved connection? Existing audits will remain.'))submit('disconnect',event.currentTarget);});
   }
