...
- **Operação do cliente:** o administrador usa seu próprio login e abre a operação pelo botão **Abrir operação** em **Empresas**. Os atalhos de fila, faturas, relatórios, exceções, histórico e configurações ficam ocultos enquanto ele estiver nas telas de gestão e aparecem somente depois que uma empresa é aberta. Um aviso identifica a empresa e o acesso administrativo. Revisões e reenvios registram e-mail, ID e papel do autor, visíveis no histórico de revisões do cliente.
