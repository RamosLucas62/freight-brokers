...
Set the required `GOOGLE_CHAT_LEADS_WEBHOOK_URL` to the leads space webhook and the required `GOOGLE_CHAT_ERRORS_WEBHOOK_URL` to a separate operational-errors space webhook. The real URLs are deployment secrets and must never be committed. The server refuses to start without both webhooks so a missing configuration cannot silently disable alerts. Temporary Google Chat failures are retried; permanent failures and exhausted lead-notification retries are recorded as structured errors. Lead alerts use readable event-specific copy when a customer requests the free audit, receives the result, receives each follow-up, signs, asks to cancel, or changes plan. Every structured application error is sent to the errors space with a readable location, stable code, correlation reference and timestamp, without document contents or secrets.

Checkout from a private audit result opens Stripe directly after plan and billing-period selection. Contractual consent is collected later, after the customer's first authenticated portal login and before product onboarding or customer data access. Apply `db/migrations/031_portal_legal_acceptance.sql` before deploying this flow.

The production follow-up cadence remains D+1, D+3, D+5, D+10 and D+30. For an intentional delivery test, first apply migration 030 and then run `scripts/replay-free-audit-followups-2min.sql` in the Supabase SQL Editor. It resets only the latest completed audit for the email declared inside the script, blocks subscribed or unsubscribed leads, creates a new delivery sequence for Resend idempotency, and schedules the five messages at 2, 4, 6, 8 and 10 minutes.
