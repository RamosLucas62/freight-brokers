...
vi.mock('../../src/notifications/google-chat.sender.js',()=>({notifySupportRequest:mocks.notifySupportRequest,notifyLeadFunnel:mocks.notifyLeadFunnel,notifyOperationalError:mocks.notifyOperationalError}));
vi.mock('../../src/free-audit/support.js',()=>({answerPortalSupport:mocks.answerPortalSupport}));
...
beforeEach(async()=>{
 ...
 mocks.notifySupportRequest.mockResolvedValue(undefined);mocks.notifyLeadFunnel.mockResolvedValue(undefined);mocks.notifyOperationalError.mockResolvedValue(undefined);
 mocks.answerPortalSupport.mockResolvedValue({answer:'Open Settings & billing to manage report recipients.',offerHuman:false});
 mocks.from.mockImplementation(table=>{
  if(['audit_admins','audit_portal_users'].includes(table))return{select:()=>({eq:()=>({maybeSingle:()=>Promise.resolve({data:table==='audit_admins'?(mocks.isAdmin?{user_id:'user-1'}:null):{enabled:mocks.enabled,guide_completed_at:mocks.guideCompletedAt},error:null})})})};
...
afterEach(async()=>{vi.unstubAllEnvs();vi.restoreAllMocks();server.closeAllConnections();await new Promise<void>(r=>server.close(()=>r()));});
const headers={'Content-Type':'application/json',Origin:'https://portal.example.com',Cookie:'audit_session=valid-token'};
it('serves login, mandatory legal acceptance, product guide and authenticated AI support with restrictive CSP',async()=>{const r=await fetch(base+'/');expect(r.status).toBe(200);expect(r.headers.get('content-security-policy')).toContain("frame-ancestors 'none'");const html=await r.text();expect(html).toContain('type="email"');expect(html).toContain('id="legal-acceptance"');expect(html).toContain('id="accept-terms"');expect(html).toContain('id="accept-privacy"');expect(html).toContain('id="product-guide"');expect(html).toContain('id="open-guide"');expect(html).toContain('id="open-support"');expect(html).toContain('id="support-panel"');});
it('hides customer workspace links while an administrator is managing accounts',async()=>{const page=await fetch(base+'/');const html=await page.text();expect(html).toContain('id="workspace-label"');expect(html).toContain('id="workspace-nav"');const admin=await fetch(base+'/admin.js');const script=await admin.text();expect(script).toContain('setWorkspaceNavigation(false)');expect(script).toContain('setWorkspaceNavigation(true)');});
...
it('keeps portal AI support behind the authenticated company boundary',async()=>{const r=await fetch(base+'/api/portal/support?company='+tenant);expect(r.status).toBe(401);expect(mocks.answerPortalSupport).not.toHaveBeenCalled();expect(mocks.from).not.toHaveBeenCalled();});
it('answers portal support only after session and company access checks',async()=>{mocks.rpc.mockImplementation(async name=>name==='portal_support_open_conversation'?{data:{id:other,status:'ai'},error:null}:{data:{id:'message-1'},error:null});const body={messages:[{role:'user',content:'Where do I update report recipients?'}],context:{view:'settings'}};const r=await fetch(base+'/api/portal/support?company='+tenant,{method:'POST',headers,body:JSON.stringify(body)});expect(r.status).toBe(200);expect(await r.json()).toEqual({answer:'Open Settings & billing to manage report recipients.',offer_human:false,conversation_id:other,status:'ai'});expect(mocks.answerPortalSupport).toHaveBeenCalledWith(body,expect.any(Function),{tenantId:tenant,subjectType:'support_conversation',subjectId:other});expect(mocks.rpc).toHaveBeenCalledWith('portal_support_append_message',expect.objectContaining({p_author_type:'customer',p_body:'Where do I update report recipients?'}));});
it('escalates a persisted conversation and alerts the dedicated support channel once',async()=>{mocks.rpc.mockImplementation(async name=>name==='portal_support_escalate'?{data:{id:other,notified_at:null,first_response_due_at:'2026-09-21T13:15:00.000Z',customer_name:'Client',customer_email:'client@example.com',escalation_problem:'I cannot resubmit this blocked invoice.'},error:null}:{data:null,error:null});const r=await fetch(base+`/api/portal/support/escalate?company=${tenant}`,{method:'POST',headers,body:JSON.stringify({conversation_id:other,problem:'I cannot resubmit this blocked invoice.'})});expect(r.status).toBe(200);expect(await r.json()).toMatchObject({conversation_id:other,status:'waiting'});expect(mocks.notifySupportRequest).toHaveBeenCalledWith(expect.objectContaining({conversationId:other,email:'client@example.com',accountName:'Company',problem:'I cannot resubmit this blocked invoice.'}));expect(mocks.rpc).toHaveBeenCalledWith('portal_support_mark_notified',{p_conversation:other});});
...
it('requires current legal acceptance before customer onboarding and data access',async()=>{mocks.legalAcceptedAt=null;let r=await fetch(base+'/api/portal/me',{headers});expect(r.status).toBe(200);expect(await r.json()).toMatchObject({requires_legal_acceptance:true,terms_version:'2026-09-12',privacy_version:'2026-09-12',show_guide:true});r=await fetch(base+`/api/portal/jobs?company=${tenant}`,{headers});expect(r.status).toBe(428);expect(await r.json()).toEqual({error:'Accept the Terms of Service and acknowledge the Privacy Policy to continue.'});});
it('records authenticated portal acceptance with immutable evidence fields',async()=>{mocks.legalAcceptedAt=null;mocks.rpc.mockResolvedValue({data:'2026-09-12T18:30:00.000Z',error:null});const r=await fetch(base+'/api/portal/legal/accept',{method:'POST',headers:{...headers,'User-Agent':'Portal Test Browser'},body:JSON.stringify({terms_accepted:true,privacy_acknowledged:true})});expect(r.status).toBe(200);expect(await r.json()).toMatchObject({ok:true,terms_version:'2026-09-12',privacy_version:'2026-09-12'});expect(mocks.rpc).toHaveBeenCalledWith('record_portal_legal_acceptance',expect.objectContaining({p_user:'user-1',p_terms_version:'2026-09-12',p_privacy_version:'2026-09-12',p_acceptance_text:expect.stringContaining('authorized'),p_user_agent:'Portal Test Browser'}));});
it('does not allow the legal endpoint to record unchecked consent',async()=>{mocks.legalAcceptedAt=null;const r=await fetch(base+'/api/portal/legal/accept',{method:'POST',headers,body:JSON.stringify({terms_accepted:false,privacy_acknowledged:true})});expect(r.status).toBe(400);expect(mocks.rpc).not.toHaveBeenCalledWith('record_portal_legal_acceptance',expect.anything());});
...
it('keeps Rose Rocket setup closed without secure credential storage',async()=>{
 const r=await fetch(base+`/api/portal/integrations/rose-rocket?company=${tenant}`,{method:'POST',headers,body:'{}'});
 expect(r.status).toBe(503);expect(mocks.from).not.toHaveBeenCalledWith('audit_rose_connections');
});
it('accepts only an owner or billing administrator for Rose Rocket setup',async()=>{
 vi.stubEnv('ROSE_ROCKET_CONNECT_ENABLED','true');rose.role='viewer';
 const r=await fetch(base+`/api/portal/integrations/rose-rocket?company=${tenant}`,{method:'POST',headers,body:'{}'});
...
it('does not store Rose Rocket credentials when the provider rejects them',async()=>{
 vi.stubEnv('ROSE_ROCKET_CONNECT_ENABLED','true');vi.stubEnv('ROSE_ROCKET_CREDENTIAL_KEY',Buffer.alloc(32,7).toString('base64url'));
 vi.spyOn(RoseRocketClient.prototype,'verifyAccess').mockRejectedValue(new Error('ROSE_ACCESS_HTTP_403'));
 const r=await fetch(base+`/api/portal/integrations/rose-rocket?company=${tenant}`,{method:'POST',headers,body:JSON.stringify({org_id:other,user_id:'33333333-3333-4333-8333-333333333333',client_id:'client-1',client_secret:'invalid-secret'})});
 expect(r.status).toBe(422);expect(rose.lastWrite).toBeNull();
});
...
it('accepts a completed card-backed free trial and records its end date',async()=>{
 ...
 expect(mocks.rpc).toHaveBeenCalledWith('sync_onboarding_billing_state',{p_session_id:'cs_test_trial',p_subscription_id:'sub_trial',p_status:'trialing',p_trial_ends_at:new Date(trialEnd*1000).toISOString()});
...
it('accepts a Stripe trial even when Checkout reports the saved card as paid',async()=>{
 ...
 expect(response.status).toBe(200);
 expect(mocks.rpc).toHaveBeenCalledWith('sync_onboarding_billing_state',{p_session_id:'cs_test_paid_trial',p_subscription_id:'sub_trial',p_status:'trialing',p_trial_ends_at:new Date(trialEnd*1000).toISOString()});
});
it('assigns the plan from the live Stripe subscription price instead of stale Checkout metadata',async()=>{
 vi.stubEnv('STRIPE_PRICE_SCALE_SEMIANNUAL','price_scale_six_months');const trialEnd=Math.floor(Date.now()/1000)+86400;
...
it.each([true,false])('reports email intake availability (%s) without changing report settings',async emailEnabled=>{
 mocks.rpc.mockImplementation(async name=>({data:name==='email_intake_enabled'?emailEnabled:null,error:null}));
 ...
 const r=await fetch(base+`/api/portal/settings?company=${tenant}`,{headers});expect(r.status).toBe(200);const result=await r.json();expect(result.notifications).toMatchObject({email_intake_enabled:emailEnabled,audit_email:'company@audit.aiolympian.com',max_recipients:3,max_senders:1});expect(result.billing).toMatchObject({plan_code:'core',billing_period:'monthly',included_invoices:500,overage_unit_amount_cents:75});expect(mocks.retrieveSubscription).not.toHaveBeenCalled();expect(mocks.rpc).not.toHaveBeenCalledWith('sync_billing_plan_from_stripe',expect.anything());
});
...
it('denies every administration route to customers',async()=>{for(const route of ['companies','users','confidence','costs','activity','action']){const r=await fetch(base+'/api/portal/admin/'+route,{headers});expect(r.status).toBe(403);}expect(mocks.rpc).not.toHaveBeenCalled();});
...
it('loads cost attribution only for administrators with a bounded date range',async()=>{mocks.isAdmin=true;mocks.rpc.mockResolvedValue({data:{summary:{cost_usd:0.12},by_tenant:[],by_operation:[]},error:null});const from='2026-09-01T00:00:00.000Z',to='2026-10-01T00:00:00.000Z';const r=await fetch(base+`/api/portal/admin/costs?company=${tenant}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,{headers});expect(r.status).toBe(200);expect(mocks.rpc).toHaveBeenCalledWith('portal_admin_costs',{p_actor:'user-1',p_tenant:tenant,p_from:from,p_to:to});});
it('lets an administrator list, open and reply to support conversations',async()=>{mocks.isAdmin=true;mocks.rpc.mockImplementation(async name=>name==='portal_admin_support_queue'?{data:{rows:[{id:other,status:'waiting'}],total:1,page:0},error:null}:name==='portal_admin_support_conversation'?{data:{conversation:{id:other,status:'waiting'},messages:[]},error:null}:{data:{id:'message-1',body:'We are checking this now.'},error:null});let r=await fetch(base+'/api/portal/admin/support',{headers});expect(r.status).toBe(200);expect(await r.json()).toMatchObject({total:1});r=await fetch(base+`/api/portal/admin/support/${other}`,{headers});expect(r.status).toBe(200);r=await fetch(base+`/api/portal/admin/support/${other}/reply`,{method:'POST',headers,body:JSON.stringify({message:'We are checking this now.'})});expect(r.status).toBe(200);expect(mocks.rpc).toHaveBeenCalledWith('portal_admin_support_reply',{p_actor:'user-1',p_conversation:other,p_body:'We are checking this now.'});});
...
it('checks target company before provisioning user',async()=>{mocks.isAdmin=true;const original=mocks.from.getMockImplementation()!;mocks.from.mockImplementation(table=>table==='audit_tenants'?{select:()=>({eq:()=>({maybeSingle:async()=>({data:null,error:null})})})}:original(table));const r=await fetch(base+'/api/portal/admin/action',{method:'POST',headers,body:JSON.stringify({action:'user.create',email:'new@example.com',company_id:tenant})});expect(r.status).toBe(409);expect(mocks.createUser).not.toHaveBeenCalled();});
...
it('lists providers without exposing credentials and scopes the read to the selected company',async()=>{
 const r=await fetch(base+`/api/portal/integrations?company=${tenant}`,{headers});expect(r.status).toBe(200);
 const data=await r.json();expect(data.providers).toHaveLength(7);expect(data.providers.find((p:any)=>p.id==='tai')).toMatchObject({setup_available:false,can_manage:true});expect(tms.filters).toContainEqual(['tenant_id',tenant]);
});
it('rejects cross-tenant integration reads and writes',async()=>{
 for(const method of ['GET','POST']){const r=await fetch(base+`/api/portal/integrations${method==='POST'?'/tai/connect':''}?company=${other}`,{method,headers,...(method==='POST'?{body:'{}'}:{})});expect(r.status).toBe(403);}
 expect(tms.lastWrite).toBeNull();
});
it('verifies Tai access before saving encrypted tenant-bound credentials',async()=>{
 vi.stubEnv('TMS_CREDENTIAL_KEY',Buffer.alloc(32,9).toString('base64url'));const verify=vi.spyOn(TaiClient.prototype,'verifyAccess').mockResolvedValue();
 const r=await fetch(base+`/api/portal/integrations/tai/connect?company=${tenant}`,{method:'POST',headers,body:JSON.stringify({site:'Acme',api_key:'private-key'})});
 expect(r.status).toBe(200);expect(await r.json()).toEqual({status:'verified'});expect(verify).toHaveBeenCalledOnce();expect(tms.lastWrite).toMatchObject({tenant_id:tenant,provider:'tai',status:'verified',account_label:'acme.taicloud.net'});expect(decryptTmsCredentials(tenant,'tai',tms.lastWrite.credentials_ciphertext)).toEqual({site:'acme',api_key:'private-key'});
});
it('does not overwrite Tai credentials when verification fails',async()=>{
 vi.stubEnv('TMS_CREDENTIAL_KEY',Buffer.alloc(32,9).toString('base64url'));vi.spyOn(TaiClient.prototype,'verifyAccess').mockRejectedValue(new Error('provider echoed SECRET'));
 const r=await fetch(base+`/api/portal/integrations/tai/connect?company=${tenant}`,{method:'POST',headers,body:JSON.stringify({site:'acme',api_key:'private-key'})});expect(r.status).toBe(422);expect(await r.text()).not.toContain('SECRET');expect(tms.lastWrite).toBeNull();
});
it('checks storage readiness before contacting Tai',async()=>{
 const verify=vi.spyOn(TaiClient.prototype,'verifyAccess');const r=await fetch(base+`/api/portal/integrations/tai/connect?company=${tenant}`,{method:'POST',headers,body:'{}'});expect(r.status).toBe(503);expect(verify).not.toHaveBeenCalled();expect(tms.lastWrite).toBeNull();
});
it('rejects viewer, global-admin customer mutations and missing required MFA',async()=>{
 for(const role of ['viewer','admin','mfa']){
  rose.role=role==='viewer'?'viewer':'owner';mocks.isAdmin=role==='admin';vi.stubEnv('REQUIRE_MFA_SENSITIVE',role==='mfa'?'true':'false');
  const r=await fetch(base+`/api/portal/integrations/turvo/request?company=${tenant}`,{method:'POST',headers,body:'{}'});expect(r.status).toBe(403);
 }
 expect(tms.lastWrite).toBeNull();
});
...
it('restricts the TMS administrative queue to global administrators',async()=>{
 ...
 r=await fetch(base+'/api/portal/admin/integrations',{headers});expect(r.status).toBe(200);expect(await r.json()).toEqual({rows:[],total:0});
});
it('enables Rose setup with a valid key unless explicitly paused',async()=>{
