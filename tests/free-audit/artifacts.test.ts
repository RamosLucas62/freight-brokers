...
describe('free audit email artifacts',()=>{
 ...
 it('explains processing failures without exposing internal details',()=>{const email=failureEmail('Alex','invalid_document','https://example.com/audit');expect(email.subject).toContain('Action needed');expect(email.html).toContain('standard, unencrypted PDF');expect(email.html).not.toContain('stack');});
