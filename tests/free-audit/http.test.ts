...
const mocks=vi.hoisted(()=>({
 ...
 answerSupport:vi.fn(),
 publicResult:vi.fn(),recordFunnelEvent:vi.fn(),recordCheckoutStarted:vi.fn(),recordCheckoutAcceptance:vi.fn(),unsubscribe:vi.fn(),
}));
...
vi.mock('../../src/free-audit/support.js',()=>({answerSupport:mocks.answerSupport}));
...
describe('free audit public boundary',()=>{
 it('creates Stripe checkout from the public pricing origin',async()=>{
  const response=await fetch(base+'/checkout',{method:'POST',headers:{Origin:'https://aiolympian.com','Content-Type':'application/json','User-Agent':'Consent Test'},body:JSON.stringify({email:'Buyer@Example.com',plan:'growth',period:'annual',terms_accepted:true,turnstile_token:'verified'})});
  expect(response.status).toBe(200);expect(response.headers.get('access-control-allow-origin')).toBe('https://aiolympian.com');expect(await response.json()).toEqual({url:'https://checkout.stripe.test/session'});expect(mocks.recordCheckoutAcceptance).toHaveBeenCalledWith(expect.objectContaining({email:'buyer@example.com',plan:'growth',period:'annual',termsVersion:'2026-09-12',source:'public_pricing',userAgent:'Consent Test'}));expect(mocks.createCheckoutSession).toHaveBeenCalledWith('buyer@example.com','growth','annual',undefined,'22222222-2222-4222-8222-222222222222');
 });
 it('requires affirmative terms acceptance before opening checkout',async()=>{const response=await fetch(base+'/checkout',{method:'POST',headers:{Origin:'https://aiolympian.com','Content-Type':'application/json'},body:JSON.stringify({email:'buyer@example.com',plan:'core',period:'monthly',turnstile_token:'verified'})});expect(response.status).toBe(400);expect(mocks.recordCheckoutAcceptance).not.toHaveBeenCalled();expect(mocks.createCheckoutSession).not.toHaveBeenCalled();});
 it('publishes versioned legal documents',async()=>{const terms=await fetch(base+'/terms');expect(terms.status).toBe(200);expect(await terms.text()).toContain('Terms of Service');const privacy=await fetch(base+'/privacy');expect(privacy.status).toBe(200);expect(await privacy.text()).toContain('evidence of legal acceptance');});
 ...
 it('answers bounded support questions only from the private page origin',async()=>{const response=await fetch(base+'/api/support',{method:'POST',headers:{Origin:'https://api.audit.aiolympian.com','Content-Type':'application/json'},body:JSON.stringify({messages:[{role:'user',content:'How does the free audit work?'}]})});expect(response.status).toBe(200);expect(await response.json()).toEqual({answer:'The free audit is an invoice-only snapshot.'});expect(mocks.answerSupport).toHaveBeenCalledWith({messages:[{role:'user',content:'How does the free audit work?'}]});const rejected=await fetch(base+'/api/support',{method:'POST',headers:{Origin:'https://evil.example','Content-Type':'application/json'},body:JSON.stringify({messages:[{role:'user',content:'Hello'}]})});expect(rejected.status).toBe(403);});
 ...
 it('renders benefits, FAQ, plans and AI support while highlighting the recommendation',async()=>{const token='d'.repeat(43);mocks.publicResult.mockResolvedValue({id:'11111111-1111-4111-8111-111111111111',company_name:'Acme',recommended_plan:'growth',result:{run_id:'r',generated_at:new Date().toISOString(),total_invoices_processed:6,total_exceptions:2,valor_total_under_review:10855,exceptions:[{invoice_id:'i',tipo_regra:'DUPLICATE',rule_label:'Duplicate billing',valor_envolvido:100,descricao:'review',source_reference:{file:'private.pdf',page:1},metadata:{}}]}});const response=await fetch(base+`/free-audit/result?token=${token}`);const html=await response.text();expect(response.status).toBe(200);expect(html).toContain('$10,855');expect(html).toContain('Invoice + rate con + POD');expect(html).toContain('Recover revenue you earned');expect(html).toContain('Your invoices are confidential');expect(html).toContain('What exactly does Olympian audit?');expect(html).toContain('Core');expect(html).toContain('Growth');expect(html).toContain('Scale');expect(html).toContain('Recommended for your volume');expect(html).toContain('Ask Olympian');expect(html).toContain('Do not send invoices, rate confirmations, PODs');expect(html).toContain("fetch('/api/support'");expect(html).not.toContain('private.pdf');expect(mocks.recordFunnelEvent).toHaveBeenCalledWith(expect.any(String),'result_opened');});
 ...
 it('creates Stripe checkout directly from the private result',async()=>{const token='f'.repeat(43);mocks.publicResult.mockResolvedValue({id:'11111111-1111-4111-8111-111111111111',email:'lead@example.com',company_name:'Acme',recommended_plan:'growth',result:{run_id:'r',generated_at:new Date().toISOString(),total_invoices_processed:1,total_exceptions:0,valor_total_under_review:0,exceptions:[]}});const response=await fetch(base+`/free-audit/result/checkout?token=${token}`,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'plan=scale&period=annual&terms_accepted=on',redirect:'manual'});expect(response.status).toBe(303);expect(response.headers.get('location')).toBe('https://checkout.stripe.test/session');expect(mocks.createCheckoutSession).toHaveBeenCalledWith('lead@example.com','scale','annual',`https://api.audit.aiolympian.com/free-audit/result?token=${token}`,'22222222-2222-4222-8222-222222222222');expect(mocks.recordFunnelEvent).toHaveBeenCalledWith(expect.any(String),'checkout_started',expect.objectContaining({plan:'scale',period:'annual',recommended_plan:'growth'}));});
 it('returns a Stripe URL to the interactive private checkout',async()=>{const token='h'.repeat(43);mocks.publicResult.mockResolvedValue({id:'11111111-1111-4111-8111-111111111111',email:'lead@example.com',company_name:'Acme',recommended_plan:'scale',result:{run_id:'r',generated_at:new Date().toISOString(),total_invoices_processed:1,total_exceptions:0,valor_total_under_review:0,exceptions:[]}});const response=await fetch(base+`/free-audit/result/checkout?token=${token}`,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/x-www-form-urlencoded'},body:'plan=scale&period=semiannual&terms_accepted=on'});expect(response.status).toBe(200);expect(await response.json()).toEqual({url:'https://checkout.stripe.test/session'});expect(mocks.createCheckoutSession).toHaveBeenCalledWith('lead@example.com','scale','semiannual',expect.stringContaining(token),'22222222-2222-4222-8222-222222222222');});
 it('keeps checkout controls in one real form and orders pricing before comparison and FAQ',async()=>{const token='g'.repeat(43);mocks.publicResult.mockResolvedValue({id:'11111111-1111-4111-8111-111111111111',email:'lead@example.com',company_name:'Acme',recommended_plan:'core',result:{run_id:'r',generated_at:new Date().toISOString(),total_invoices_processed:1,total_exceptions:0,valor_total_under_review:0,exceptions:[]}});const response=await fetch(base+`/free-audit/result?token=${token}`);const html=await response.text();const form=html.slice(html.indexOf('<form class="checkout-form"'),html.indexOf('</form>',html.indexOf('<form class="checkout-form"'))+7);expect(form).toContain('name="period"');expect(form).toContain('name="terms_accepted" required');expect(form).toContain('Terms of Service');expect(form).toContain('Privacy Policy');expect(form).toContain('name="plan" value="core"');expect(form).toContain('name="plan" value="growth"');expect(form).toContain('name="plan" value="scale"');expect(form).toContain('data-period="monthly">$497');expect(form).toContain('data-period="semiannual">$2,682');expect(form).toContain('data-period="semiannual">$8,082');expect(form).toContain('Save 10% vs monthly');expect(form).toContain('data-period="annual">$4,970');expect(form).toContain('Save 17% vs monthly');expect(form.match(/<span class="trial-note">7 days free<\/span>/g)).toHaveLength(9);expect(html).toContain("Accept:'application/json'");expect(response.headers.get('content-security-policy')).toMatch(/script-src 'nonce-[A-Za-z0-9_-]+'/);expect(html.match(/href="#plans"/g)).toHaveLength(2);expect(html.indexOf('Choose ongoing coverage')).toBeLessThan(html.indexOf('What changes between plans?'));expect(html.indexOf('What changes between plans?')).toBeLessThan(html.indexOf('Everything you need to know.'));});
});
