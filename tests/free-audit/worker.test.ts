...
describe('free audit worker delivery',()=>{
 ...
 it('sends the scheduled D+30 follow-up with result and unsubscribe links',async()=>{mocks.claimFollowup.mockResolvedValue({request_id:request.id,day_offset:30,email:request.email,contact_name:request.contact_name,company_name:request.company_name,loads_per_month:'2000',recommended_plan:'scale',result:report});const sender={send:vi.fn().mockResolvedValue('email-30')};await expect(processFreeAuditFollowup(sender as any,'https://audit.example.com','secret')).resolves.toBe(true);expect(sender.send).toHaveBeenCalledWith(expect.objectContaining({idempotencyKey:`free-audit-followup-${request.id}-30`,html:expect.stringMatching(/free-audit\/result\?token=.*free-audit\/unsubscribe\?token=/s),attachments:[]}));expect(mocks.finishFollowup).toHaveBeenCalledWith(request.id,30);});
});
