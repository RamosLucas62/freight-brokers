...
describe('shared rate-limit boundary',()=>{
 ...
 it('uses only the nearest valid hop added by the EasyPanel proxy',()=>{vi.stubEnv('TRUST_PROXY','easypanel');expect(clientIp({'x-forwarded-for':'198.51.100.8, 203.0.113.10'},'172.18.0.2')).toBe('203.0.113.10');});
 it('rejects forged or malformed proxy IP values',()=>{vi.stubEnv('TRUST_PROXY','easypanel');expect(clientIp({'x-forwarded-for':'not-an-ip'},'172.18.0.2')).toBe('172.18.0.2');vi.stubEnv('TRUST_PROXY','cloudflare');expect(clientIp({'cf-connecting-ip':'not-an-ip'},'172.18.0.2')).toBe('172.18.0.2');});
});
