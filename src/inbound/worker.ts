...
export async function processJob(job:repository.InboundJob,client:ResendReceivingClient|TmsReceivingClient) {
 ...
 try {
  ...
  if(previous){await repository.finish(job,'completed',previous,null);if(job.source==='tms')await repository.validateTmsIntake(job);info('inbound.worker.completed',{job_id:job.id,tenant_id:job.tenant_id,reused_report:true,duration_ms:Date.now()-started});return;}
  stage='account_check';
  ...
  for(const path of paths){const id=basename(path,'.pdf');const attachment=pdfs.find(item=>item.id===id)!;const stored=cached.get(id);let documentType:DocumentType;
   ...
   if(documentType==='accessorial_evidence'){stage='extract_accessorial_evidence';const result=stored?.extraction?AccessorialExtractionSchema.parse(stored.extraction):await new OpenRouterAccessorialExtractor().extract(path,costContext);result.source_file=labels[path];if(!stored?.extraction)await repository.cacheSupportingExtraction(job,id,'accessorial_evidence',result);accessorialEvidence.push(result);continue;}
  ...
  const report=await runAuditPipeline({tenantId:job.tenant_id,filePaths:invoicePaths,sourceLabels:labels,pods,rateConfirmations,accessorialEvidence,reconcileSupportingDocuments:true,requireTmsEvidence:job.source==='tms',
   ...
   getCarrier,store,extractor:{async extract(path,context){
    ...
    if(stored){const parsed=ExtractionResultSchema.parse(stored);await billInvoice(id);return parsed;}
    const result=await extractor.extract(path,context);
    await repository.cacheExtraction(job,id,result);
    await billInvoice(id);
    return result;
   }},
  });
  ...
  stage='finish_job';await repository.finish(job,'completed',report,null);if(job.source==='tms')await repository.validateTmsIntake(job);info('inbound.worker.completed',{job_id:job.id,tenant_id:job.tenant_id,duration_ms:Date.now()-started,pdf_count:pdfs.length,pod_count:pods.length,rate_confirmation_count:rateConfirmations.length,invoice_count:report.total_invoices_processed,exception_count:report.total_exceptions});
 }catch(error){
  const detail=error instanceof Error?error.message:'unknown_error';
  const status=typeof error==='object' && error!==null && '$metadata' in error
   ? (error as {$metadata?:{httpStatusCode?:number}}).$metadata?.httpStatusCode
   : undefined;
  if(error instanceof IncompleteTmsEvidence){await repository.recordEvidenceIssues(job,error.issues);await repository.finish(job,'needs_review',null,'TMS_EVIDENCE_INCOMPLETE');return;}
