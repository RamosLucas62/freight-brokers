...
function page(res:ServerResponse,status:number,content:string,title='Olympian free audit'){
 ...
 res.writeHead(status,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','Referrer-Policy':'no-referrer','Content-Security-Policy':`default-src 'none'; style-src 'nonce-${nonce}'; img-src 'self' data:; form-action 'self'; base-uri 'none'; frame-ancestors 'none'`,'Content-Length':String(Buffer.byteLength(html))});res.end(html);
}
...
function resultPage(result:repositoryResult,token:string){
 ...
 return async(req:IncomingMessage,res:ServerResponse,limiter:RateLimiter):Promise<boolean>=>{
  ...
  if(resultCheckout&&req.method==='POST'){
   ...
   try{
    ...
    await repository.recordFunnelEvent(audit.id,'checkout_started',{plan:input.plan,period:input.period,recommended_plan:audit.recommended_plan}).catch(()=>{});info('free_audit.checkout.created',{request_id:traceId,audit_request_id:audit.id,plan:input.plan,period:input.period});res.writeHead(303,{Location:session.url,'Cache-Control':'no-store'});res.end();return true;
  ...
  if((result||continueResult)&&req.method==='GET'){
   try{
    ...
    await repository.recordFunnelEvent(audit.id,'result_opened').catch(()=>{});page(res,200,resultPage(audit,token),`Your Olympian audit — ${money(audit.result.valor_total_under_review)} under review`);return true;
