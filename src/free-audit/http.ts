...
const planCopy={core:{name:'Core',price:'$497',limit:'up to 500 invoices/month'},growth:{name:'Growth',price:'$997',limit:'up to 1,500 invoices/month'},scale:{name:'Scale',price:'$1,497',limit:'up to 3,000 invoices/month'}} as const;
 ...
 return async(req:IncomingMessage,res:ServerResponse,limiter:RateLimiter):Promise<boolean>=>{
  ...
  if((result||continueResult)&&req.method==='GET'){
   try{
    ...
    if(continueResult){await repository.recordFunnelEvent(audit.id,'pricing_viewed',{recommended_plan:audit.recommended_plan}).catch(()=>{});const target=new URL(config.offerUrl);target.searchParams.set('plan',audit.recommended_plan);res.writeHead(303,{Location:target.href,'Cache-Control':'no-store'});res.end();return true;}
