...
export async function processFreeAudit(sender:ResendSender,signupUrl:string,publicUrl=signupUrl,linkSecret=process.env.CSRF_SECRET??(process.env.NODE_ENV==='test'?'test-free-audit-link-secret-32bytes':'')):Promise<boolean>{
 ...
 try{
  ...
  void notifyLeadFunnel({stage:'audit_received',requestId:request.id,email:request.email,company:request.company_name,name:request.contact_name,metadata:{invoice_count:report.total_invoices_processed,exception_count:report.total_exceptions,recommended_plan:plan}}).catch(()=>{});
