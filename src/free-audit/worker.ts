...
export async function processFreeAudit(sender:ResendSender,signupUrl:string,publicUrl=signupUrl,linkSecret=process.env.CSRF_SECRET??(process.env.NODE_ENV==='test'?'test-free-audit-link-secret-32bytes':'')):Promise<boolean>{
 ...
 try{
  ...
  void notifyLeadFunnel({stage:'audit_received',requestId:request.id,email:request.email,company:request.company_name,name:request.contact_name,metadata:{invoice_count:report.total_invoices_processed,exception_count:report.total_exceptions,recommended_plan:plan}}).catch(error=>failure('google_chat.lead_notification.failed',error,{audit_request_id:request.id,stage:'audit_received'}));
...
export async function processFreeAuditFollowup(sender:ResendSender,publicUrl:string,linkSecret:string):Promise<boolean>{
 ...
 try{const token=resultAccessToken(followup.request_id,linkSecret);const resultUrl=new URL('/free-audit/result',publicUrl);resultUrl.searchParams.set('token',token);const unsubscribeUrl=new URL('/free-audit/unsubscribe',publicUrl);unsubscribeUrl.searchParams.set('token',token);const email=followupEmail(followup.day_offset,followup.contact_name,followup.result,followup.recommended_plan,resultUrl.href,unsubscribeUrl.href);await sender.send({idempotencyKey:`free-audit-followup-${followup.request_id}-${followup.day_offset}-${followup.delivery_sequence}`,to:[followup.email],...email,attachments:[]});await repository.finishFollowup(followup.request_id,followup.day_offset);void notifyLeadFunnel({stage:'followup_sent',requestId:followup.request_id,email:followup.email,company:followup.company_name,name:followup.contact_name,follow:followup.day_offset,metadata:{recommended_plan:followup.recommended_plan}}).catch(error=>failure('google_chat.lead_notification.failed',error,{audit_request_id:followup.request_id,stage:'followup_sent',day_offset:followup.day_offset}));info('free_audit.followup.sent',{audit_request_id:followup.request_id,day_offset:followup.day_offset});return true;}catch(error){await repository.failFollowup(followup.request_id,followup.day_offset).catch(finishError=>failure('free_audit.followup.failure_record_failed',finishError,{audit_request_id:followup.request_id,day_offset:followup.day_offset}));failure('free_audit.followup.failed',error,{audit_request_id:followup.request_id,day_offset:followup.day_offset});throw error;}
}
