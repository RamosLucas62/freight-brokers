...
export function followupEmail(day:1|3|5|10|30,name:string,report:AuditReport,plan:'core'|'growth'|'scale',resultUrl:string,unsubscribeUrl:string){
 const plans={core:'Core',growth:'Growth',scale:'Scale'};const findings=report.total_exceptions;const invoices=report.total_invoices_processed;const amount=money(report.valor_total_under_review);const selectedPlan=plans[plan];
 const copy={
  1:{subject:`Your audit found ${findings} item${findings===1?'':'s'} worth a second look`,eyebrow:'YOUR FINDINGS',heading:'Before another invoice reaches the payment queue.',paragraphs:[`Your free audit surfaced ${findings} item${findings===1?'':'s'} across ${invoices} eligible invoice${invoices===1?'':'s'}, representing ${amount} under review.`,"The point is not to create more work. It is to show your team exactly where attention is needed before a duplicate, changed payment detail, or carrier issue becomes a costly correction later."],bullets:['See every finding and the reason it was flagged','Trace the finding back to its source document','Focus review time on exceptions instead of every invoice'],cta:'Review my findings',closing:'Your private audit is ready when you are.'},
  3:{subject:'How many invoice exceptions stay invisible?',eyebrow:'THE COST OF MANUAL REVIEW',heading:'The risky invoice is rarely the obvious one.',paragraphs:['Most carrier invoices look routine. That is exactly why duplicate charges, banking changes, carrier mismatches, and missing support can blend into the daily AP workload.',"Olympian checks each incoming invoice consistently and separates exceptions from clean documents, so your team can spend its time deciding what to do—not searching for what might be wrong."],bullets:['Continuous checks as invoices arrive','Exceptions organized in one review queue','Immediate alerts for critical risks and scheduled summaries for oversight'],cta:'See continuous protection',closing:'Your free audit was the snapshot. Olympian is the process that keeps watching.'},
  5:{subject:`A clearer path for your invoice volume: ${selectedPlan}`,eyebrow:'BUILT FOR YOUR OPERATION',heading:'Turn invoice review into exception review.',paragraphs:[`Based on the volume you shared, ${selectedPlan} is the recommended starting point for your operation. It gives your team a private intake address and a repeatable audit trail without asking someone to inspect every field manually.`,"The outcome is a calmer AP workflow: clean invoices keep moving, questionable items become visible, and your team retains the final decision."],bullets:['Forward invoices to one private address','Receive prioritized findings with supporting context','Keep reports, history, and follow-up actions together'],cta:`Explore ${selectedPlan}`,closing:'Every plan includes a 7-day free trial, so you can validate the workflow with your own invoices before the first charge.'},
  10:{subject:'What would your team do with fewer invoice checks?',eyebrow:'TIME BACK FOR YOUR TEAM',heading:'Your team should decide—not hunt.',paragraphs:["Manual review asks experienced people to repeat the same search on every invoice, even when most documents are clean. Olympian changes that balance by doing the first pass continuously and presenting the exceptions that deserve judgment.","You stay in control of payment decisions. The system provides the evidence, history, and alerts needed to reach those decisions faster and more consistently."],bullets:['Reduce repetitive invoice-by-invoice checking','Surface uncertainty for review instead of making unsupported accusations','Create a visible record of findings and decisions'],cta:'Start my 7-day trial',closing:'Your audit result and recommended plan are still available at the private link.'},
  30:{subject:'Your audit expires soon—has the workload changed?',eyebrow:'FINAL CHECK-IN',heading:'The invoices kept arriving. Did the review burden change?',paragraphs:[`It has been 30 days since your free audit identified ${findings} item${findings===1?'':'s'} and ${amount} under review. If the same team is still checking every invoice manually, the underlying cost has not disappeared—it has repeated with each new batch.`,"Your private result is nearing the end of its availability. Revisit it now to review the evidence, compare coverage, and decide whether continuous auditing belongs in your workflow."],bullets:['Return to the original findings','Compare the recommended coverage for your volume','Try the complete workflow free for 7 days'],cta:'Revisit my audit',closing:'No pressure and no unsupported promises—just a chance to evaluate the workflow with your own operation.'},
 }[day];
 const bullets=copy.bullets.map(item=>`<tr><td valign="top" style="padding:0 10px 10px 0;color:#ef5427;font-weight:700">✓</td><td style="padding:0 0 10px">${escapeHtml(item)}</td></tr>`).join('');
 return {subject:copy.subject,html:`<!doctype html><html><body style="margin:0;background:#f7f7f0;color:#11110f;font-family:Arial,sans-serif;line-height:1.55"><table width="100%" role="presentation"><tr><td align="center" style="padding:38px 16px"><table width="100%" role="presentation" style="max-width:620px;background:#fff;border:2px solid #11110f"><tr><td style="padding:38px"><p style="margin:0 0 24px;color:#ef5427;font-size:13px;font-weight:800;letter-spacing:1.5px">OLYMPIAN · ${copy.eyebrow}</p><h1 style="font-size:30px;line-height:1.15;margin:0 0 22px">${copy.heading}</h1><p>Hi ${escapeHtml(name)},</p>${copy.paragraphs.map(paragraph=>`<p>${escapeHtml(paragraph)}</p>`).join('')}<table role="presentation" style="margin:22px 0 6px">${bullets}</table><p style="margin:30px 0"><a href="${escapeHtml(resultUrl)}" style="display:inline-block;background:#ef5427;color:#fff;border:2px solid #11110f;padding:14px 20px;text-decoration:none;font-weight:700">${escapeHtml(copy.cta)}</a></p><p style="font-weight:700">${escapeHtml(copy.closing)}</p><hr style="border:0;border-top:1px solid #d8d8d0;margin:28px 0 18px"><p style="font-size:12px;color:#66665f">You are receiving this because you requested an Olympian free invoice audit. <a href="${escapeHtml(unsubscribeUrl)}" style="color:#66665f">Stop these audit follow-ups</a>.</p></td></tr></table></td></tr></table></body></html>`};
}
