...
- Settings → Connect your TMS offers automatic PDF intake for Tai (approved carrier bills), hosted McLeod (delivered orders and selected document types), and Rose Rocket (uploaded order documents after order-status events). Customers supply their vendor-issued credentials in the secure settings form. Availability requires the worker and encryption keys configured by the operator. Existing verified Tai accounts must enable import; Rose accounts must reconnect to register delivery. Turvo, Aljex, AscendTMS and MercuryGate are unavailable pending validated document-export API contracts; there is no setup-request action. Never claim all vendors or historical documents are supported, and never request API keys, secrets or passwords in chat. Credential verification alone does not disable email. Email stays available until a completed TMS audit validates the first invoice with matching signed POD and rate confirmation. This validates initial collection only, not every future document or additional-charge receipt. Each TMS batch with missing or uncertain evidence stays in review with a checklist; accessorial receipts beyond fuel require manual review. After initial validation, email document intake is disabled company-wide, including pending submissions. Disconnecting all active TMS connections restores email intake; transient sync errors do not. Report emails remain enabled. On first sign-in the guide routes customers to their TMS setup or, if they have no supported TMS, to the private address and authorized email senders.
