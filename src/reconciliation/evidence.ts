...
const proven=<T>(field:PodField<T>|undefined,threshold:number):boolean=>field?.value!=null&&field.confidence>=threshold&&Boolean(field.evidence?.page&&field.evidence?.text?.trim());
/** Presence alone is not proof: bind evidence to each invoice's load and require
 * readable source evidence. Accessorial receipts are not modeled by the current
 * extractors, so those charges always require review rather than false assurance. */
export function tmsEvidenceIssues(invoices:InvoiceRecord[],pods:PodExtractionResult[],rates:RateConfirmationExtractionResult[],threshold=0.9):EvidenceIssue[]{
 ...
 return invoices.flatMap<EvidenceIssue>(invoice=>{
  ...
  const match=<T extends {fields:{load_number:PodField<string>}}>(items:T[])=>items.filter(item=>proven(item.fields.load_number,threshold)&&norm(item.fields.load_number.value??'')===norm(load));
  ...
  if(invoice.accessorials.some(item=>!['FUEL','FUELSURCHARGE','LINEHAUL'].includes(norm(item.tipo))))missing.push('ACCESSORIAL_EVIDENCE_REVIEW');
