...
const properties={carrier_name:field({type:['string','null']}),authorized_by:field({type:['string','null']}),hourly_rate:field({type:['number','null'],minimum:0}),free_minutes:field({type:['integer','null'],minimum:0}),rounding_minutes:field({type:['integer','null'],minimum:1}),maximum_amount:field({type:['number','null'],minimum:0}),load_number:field({type:['string','null']}),charge_type:field(enumeration(['LUMPER','DETENTION','LAYOVER','LIFTGATE','REDELIVERY','TONU','OTHER'])),record_kind:field(enumeration(['receipt','time_record','authorization','other'])),amount:field({type:['number','null'],minimum:0}),currency:field(enumeration(['USD','CAD','OTHER'])),service_date:field({type:['string','null']}),arrival_at:field({type:['string','null']}),departure_at:field({type:['string','null']})};
...
export class OpenRouterAccessorialExtractor{
 ...
 async extract(file:string,context?:CostContext):Promise<AccessorialExtractionResult>{
  ...
  try{
   ...
   // Completeness and contractual authorization are checked by the deterministic
   // reconciliation layer; extraction confidence alone never approves a charge.
   return AccessorialExtractionSchema.parse({source_file:file,fields,requires_human_review:Object.values(fields).some(f=>f.value!=null&&(f.confidence<.9||!f.evidence)),raw:{provider:'openrouter',model:parsed.model??model}});
