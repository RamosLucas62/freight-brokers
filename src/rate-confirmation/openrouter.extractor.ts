...
const responseSchema={type:'object',additionalProperties:false,required:['fields'],properties:{fields:{type:'object',additionalProperties:false,required:['currency','load_number','bol_number','carrier_name','origin','destination','linehaul_amount','total_amount','accessorials'],properties:{currency:field({type:['string','null'],enum:['USD','CAD',null]}),load_number:field(nullableString),bol_number:field(nullableString),carrier_name:field(nullableString),origin:field(nullableString),destination:field(nullableString),linehaul_amount:field(nullableNumber),total_amount:field(nullableNumber),accessorials:{type:'array',maxItems:100,items:{type:'object',additionalProperties:false,required:['type','description','amount','confidence','evidence'],properties:{type:{type:'string'},description:{type:'string'},amount:{type:'number',minimum:0},confidence:{type:'number',minimum:0,maximum:1},evidence}}}}}}};
...
function normalizedFallbackPayload(value:unknown):{fields:unknown}{
 ...
 return {fields:{currency:pick('currency'),load_number:pick('load_number','load_id'),bol_number:pick('bol_number','bol_id'),carrier_name:pick('carrier_name','carrier'),origin:pick('origin'),destination:pick('destination'),linehaul_amount:pick('linehaul_amount','linehaul'),total_amount:pick('total_amount','total'),accessorials}};
}
