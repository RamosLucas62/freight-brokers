...
export function createApp(deps:HttpDependencies) {
 ...
 const server=createServer(async(req,res)=>{
  ...
  res.once('finish',()=>info('http.request.completed',{request_id:traceId,method:req.method,path:new URL(req.url??'/','http://local').pathname,status_code:res.statusCode,duration_ms:Date.now()-started}));
  securityHeaders(res);
