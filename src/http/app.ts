...
import {processStripeEvent} from '../billing/repository.js';
...
export function createApp(deps:HttpDependencies) {
 ...
 const server=createServer(async(req,res)=>{
  ...
  if(req.url==='/webhooks/stripe'&&req.method==='POST'){
   ...
   try{await processStripeEvent(event);respond(200,{received:true});return;}
   catch{respond(503,{error:'temporarily_unavailable'});return;}
  }
